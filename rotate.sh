...
# 指定要查找的扩展名
extension="mp4"
folder="/mnt/c/Users/zen/Downloads/media/dance/toRight"
# 遍历当前目录下的所有文件
...
extension="mp4"
folder="/mnt/c/Users/zen/Downloads/media/dance/toLeft"
# 遍历当前目录下的所有文件
for file in "$folder"/*.$extension; do
    # 检查文件是否具有指定的扩展名
    if [[ $file == *.$extension ]]; then
        ...
        echo "文件名： $basename_without_ext"
        echo "扩展名： $ext"
        ffmpeg -i ${file} -vf transpose=2 -c:v h264_nvenc -preset medium -c:a copy -map_chapters -1 ${folder}/${aftername}
        echo "旧文件名: ${filename} 新文件名: ${aftername}"
        echo "done"
    fi
done
